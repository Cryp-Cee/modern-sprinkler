...
public class IronSprinklerHeadBlockEntity extends BlockEntity implements GeoAnimatable {
    ...
    private boolean isActiveServer() {
        ...
        // Prüfe Pumpe darunter → hat Core?
        BlockEntity below = this.level.getBlockEntity(this.worldPosition.below());
        if (below instanceof SprinklerPumpBlockEntity pump) {
            return pump.hasCore();
        }
        return false;
    }
    ...
    public double getTick(Object animatable) {
        return AnimationTickHolder.getTick(this);
    }

    public float getRotationDeg() {
        return Mth.wrapDegrees(this.rotationDeg);
    }
}
