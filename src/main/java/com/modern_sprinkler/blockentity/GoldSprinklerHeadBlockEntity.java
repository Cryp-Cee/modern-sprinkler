...
public class GoldSprinklerHeadBlockEntity extends BlockEntity implements GeoAnimatable {

    private static final int SPRAY_INTERVAL_TICKS = 5;
    private static final int RADIUS_BLOCKS = 2;      // Gold → 5x5
    ...
    public GoldSprinklerHeadBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.GOLD_SPRINKLER_HEAD_BE.get(), pos, state);
    }

    public static void serverTick(Level level, BlockPos pos, BlockState state, GoldSprinklerHeadBlockEntity be) {
    ...
    public static void clientTick(Level level, BlockPos pos, BlockState state, GoldSprinklerHeadBlockEntity be) {
        if (level == null || !level.isClientSide) return;
    ...
    private boolean isActiveServer() {
        ...
        if (!this.getBlockState().is(ModBlocks.GOLD_SPRINKLER_HEAD_ACTIVE.get())) return false;
    ...
    private boolean isActiveClient() {
        return this.getBlockState().is(ModBlocks.GOLD_SPRINKLER_HEAD_ACTIVE.get());
    }
    ...
    private <E extends GoldSprinklerHeadBlockEntity> PlayState predicate(AnimationState<E> s) {
