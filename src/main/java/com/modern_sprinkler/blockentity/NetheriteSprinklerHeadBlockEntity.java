...
public class NetheriteSprinklerHeadBlockEntity extends BlockEntity implements GeoAnimatable {

    private static final int SPRAY_INTERVAL_TICKS = 5;
    private static final int RADIUS_BLOCKS = 4;      // Netherite → 9x9
    ...
    public NetheriteSprinklerHeadBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.NETHERITE_SPRINKLER_HEAD_BE.get(), pos, state);
    }

    public static void serverTick(Level level, BlockPos pos, BlockState state, NetheriteSprinklerHeadBlockEntity be) {
    ...
    public static void clientTick(Level level, BlockPos pos, BlockState state, NetheriteSprinklerHeadBlockEntity be) {
        if (level == null || !level.isClientSide) return;
    ...
    private boolean isActiveServer() {
        ...
        if (!this.getBlockState().is(ModBlocks.NETHERITE_SPRINKLER_HEAD_ACTIVE.get())) return false;
    ...
    private boolean isActiveClient() {
        return this.getBlockState().is(ModBlocks.NETHERITE_SPRINKLER_HEAD_ACTIVE.get());
    }
    ...
    private <E extends NetheriteSprinklerHeadBlockEntity> PlayState predicate(AnimationState<E> s) {
